...
function get_build_images_from_docker()
{
    local image_name=$1

    filter="${registry_server}/${package}/${image_name}"
    image_tags=`docker image ls --filter=reference=${filter} --format "{{.Tag}}"`
...
function remove_image_to_docker()
{
    local package=$1
    local image_name=$2
    ...
    for key in ${!match_map[@]}; do
        if [[ ${match_map[${key}]} -eq 1 ]]; then
            echo "docker image rm ${registry_server}/${package}/${image_name}:${key}"
            docker image rm ${registry_server}/${package}/${image_name}:${key}
        fi
    done
}
...
function proc_chart_job()
{
    ...
    for chart in ${tmp_dir}/${package}/charts/*; do
        ...
        get_build_images_from_docker ${image_name}
        ...
        remove_image_to_docker ${package} ${image_name}
        echo ""
        echo "============================================="
        echo ""
